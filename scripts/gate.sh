...
# --- Python surface ---
# This repo keeps its Python project under evaluator/ (a self-contained subproject with its own
# pyproject.toml), so the gate runs the Python checks there. Falls back to a root-level project.
PYDIR=""
if   [ -f pyproject.toml ];           then PYDIR="."
elif [ -f evaluator/pyproject.toml ]; then PYDIR="evaluator"
fi
if [ -n "$PYDIR" ]; then
  PY="$ROOT_DIR/.venv/bin/python"; [ -x "$PY" ] || PY="python3"
  RUFF="$ROOT_DIR/.venv/bin/ruff"; [ -x "$RUFF" ] || RUFF="ruff"
  check "ruff check clean"        "(cd '$PYDIR' && '$RUFF' check .)"
  check "ruff format clean"       "(cd '$PYDIR' && '$RUFF' format --check .)"
  check "pytest green (offline)"  "(cd '$PYDIR' && '$PY' -m pytest -q -m 'not slow')"
  check "no TODO/FIXME in src/"   "! grep -rniE --include='*.py' 'TODO|FIXME|XXX' '$PYDIR/src'"
fi
