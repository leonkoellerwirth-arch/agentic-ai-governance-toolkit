...
missing was the wiring. `scripts/governance-gate.sh` is that wiring, and it depends on nothing but
the evaluator: no network, no CI service, no minutes.
...
== Organizational readiness ==
  ✗ findings — see governance-evidence/readiness.json
evidence: governance-evidence  ·  summary: governance-evidence/summary.md
GOVERNANCE GATE: FAIL
```

Pass only the checks you have; each is skipped when its flag is absent, and at least one is
required. Exit `0` clean, `1` findings, `2` misconfiguration — a misconfigured gate never degrades
into a silent pass, and neither does an unreadable input file.

**The run is the evidence.** Every check writes machine-readable JSON to the evidence directory
alongside a readable `summary.md`, both stamped with the evaluator version, the commit under test
and a UTC timestamp. A reviewer reads the summary; an auditor reads the JSON; neither has to trust
a screenshot.

`--no-fail` reports without blocking. That is a reasonable way to start on a repository that has
never been gated — and it should carry a date by which it flips back, because a gate that never
blocks is a green check that means nothing.
...
Run it wherever you run anything: a pre-commit hook, a Makefile target, GitLab CI, Jenkins, or by
hand before a release.

### On GitHub, optionally

[`action.yml`](action.yml) is a thin wrapper around the same script — the logic lives in one place
so CI and the local run cannot drift. It adds the job summary and uploads the evidence as a build
artifact; it adds nothing else, and it is not required.
...
| **Evidence** | [The evidence format](docs/08-evidence/README.md) — published schema, ruleset fingerprints, and what a digest does not prove |
